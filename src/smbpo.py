...
class SMBPO(Configurable, Module):
    ...
    def __init__(self, config, env_factory, data, islog=True):
        ...
        self.flag = 0
        self.pro = self.init_probability
    ...
    def step_generator(self):
        ...
        while True:
            ...
            if t >= self.buffer_min:
                ...
                action, flag = self.mps(policy, state)
            ...
            if done or violation or (len(episode) == max_episode_steps):
                ...
                eva = self.evaluate()
                self.pro = eva['probability'] * self.init_probability
    ...
    def update_models(self, model_steps):
        ...
        self.solver.update_r_bounds(r_min, r_max, self.pro)

    def mps(self, policy, current_state, epsilon=0.6, k=5, j=10, n=3, h=5):
        q = flag = 0
        action = policy.act1(current_state, eval=False)
        while k > 0 and j > 0:
            j = j - 1
            states = current_state.unsqueeze(0)
            with torch.no_grad():
                can_actions = policy.act(states, eval=False)
            vio = mean_v = 0
            for i in range(n):
                rets = 0
                actions = can_actions
                ...
                mean_v = (i * mean_v + rets) / (i + 1)
            if vio / n >= epsilon:
                flag = 1
                continue
            elif mean_v > q:
                action = can_actions.squeeze(0)
                q = mean_v
                k = k - 1
            else:
                k = k - 1
    ...
    def update_solver(self, update_actor=True):
        ...
        if update_actor:
            solver.update_actor_and_alpha(combined_samples[0])  # states
    ...
    def evaluate(self):
        ...
        dones = sum([traj.get('dones').sum().item() for traj in eval_traj])
        pro = dones / N_EVAL_TRAJ
